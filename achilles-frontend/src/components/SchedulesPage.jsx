...
const SchedulesPage = () => {
    ...
    const fetchSchedules = async () => {
        ...
        setNotStartedSchedules(res.filter(schedule => new Date(schedule.start) > new Date()));
        setActiveSchedules(res.filter(schedule => new Date(schedule.start) <= new Date() && new Date(schedule.end) >= new Date()));
        setEndedSchedules(res.filter(schedule => new Date(schedule.end) < new Date()));
    }
    ...
    return (
        <div>
            {formIsVisible &&
                <EventForm initValues={null} isEvent={false} onEventCreated={() => {
                    ...
                    <TableBody>
                        {activeSchedules.map(schedule => (
                            <TableRow key={schedule.id}>
                                <ScheduleWDateUpdate key={schedule.id} schedule={schedule} handleChangeEndDate={handleChangeEndDate}/>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
                ...
                <Table>
                    <TableBody>
                        {notStartedSchedules.map(schedule => (
                            <TableRow key={schedule.id}>
                                <Schedule key={schedule.id} schedule={schedule} handleDeleteSchedule={handleDeleteSchedule}/>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
