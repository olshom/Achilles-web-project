...
const Plans = () => {
  ...
  const handlePlanUpdate = async (event) => {
    ...
    const PlanWasUpdated = await PlansService.updatePlan(
      idToUpdate,
      updatedPlan,
    );
    setDescription("");
    ...
    setFormIsVisible(false);
    setPlans(
      plans.map((p) => (p.id === PlanWasUpdated.id ? PlanWasUpdated : p)),
    );
  };
  ...
  const planFormFunction = () => {
    return (
      <Paper style={{ padding: "2rem", marginBottom: "2rem" }}>
        <form
          onSubmit={idToUpdate ? handlePlanUpdate : handlePlanCreate}
          style={{
            display: "flex",
