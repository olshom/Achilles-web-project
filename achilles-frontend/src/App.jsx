import { Container, Button, Toolbar, AppBar } from "@mui/material";
...
function App() {
  ...
  const achievementId = achievementMatch
    ? Number(achievementMatch.params.id)
    : null;

  const matchUser = useMatch("/users/:id");
  const userOfClub = matchUser
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      <Container sx={{ marginTop: 2 }}>
        <Routes>
          ...
          <Route
            path="/login"
            element={!user ? <Login /> : <Navigate to="/events" replace />}
          />
          <Route path="/events" element={<Events />} />
          <Route
            path="/signup"
            element={!user ? <Signup /> : <Navigate to="/events" replace />}
          />
          <Route
            path="/members"
          ...
          <Route
            path="/admins"
            element={user?.roles.includes("admin") ? <AdminsPage /> : null}
          />
          ...
          <Route
            path="/achievements"
            element={user ? <Achievements /> : <Navigate to="/login" replace />}
          />
          <Route
