...
const buildDateFromParts = (date, time, timeZone) => {
...
const createEventsForSchedule = async (scheduleId, recurringStartDate, recurringEndDate, recurringDays, title, uniform, startTime, endTime, timeZone, coach, description, selectedGroups) => {
...
router.post("/", userExtractor, isAdmin, async (req, res) => {
    const { recurringDays, title, selectedGroups, uniform, recurringStartDate, recurringEndDate, startTime, endTime, timeZone, coach, description, } = req.body;
    ...
    const scheduleIsNotEmpty = await createEventsForSchedule(newSchedule.id, recurringStartDate, recurringEndDate, recurringDays, title, uniform, startTime, endTime, timeZone, coach, description, selectedGroups);
    if (!scheduleIsNotEmpty) {
        await newSchedule.destroy();
        return res
            .status(400)
            .json({ error: "No events can be created for the given schedule." });
    }
